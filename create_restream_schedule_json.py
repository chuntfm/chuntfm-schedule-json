...
def parse_show_info(stream_dict):
    ...
    try:
        ...
        title_re = re.search(
            "(.*?)@*\s*(chunt\s*fm)\s*(.*)", show_title.strip(), re.IGNORECASE
        )
    ...
    try:
        ...
        if start_timestamp is not None:
            ...
            start_time_uk = datetime.datetime.fromtimestamp(
                start_timestamp, tz=pytz.timezone("Europe/London")
            )
    ...
    restream["start_timestamp_uk"] = (
        start_time_uk.isoformat() if start_time_uk is not None else None
    )
    restream["end_timestamp_uk"] = (
        end_time_uk.isoformat() if end_time_uk is not None else None
    )
...
def main(debug=False):
    ...
    parser.add_argument(
        ...
        required=not debug,
        default=None,
        help="Input .json file (can be url)",
    )
    parser.add_argument(
        "-inext",
        "--input-next",
    ...
    restream = {"current": None, "next": None}
